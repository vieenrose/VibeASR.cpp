...
| **MAX-SPEED v3.2 (DEFAULT, p2)** | `VAE_FILE=vae-encoder-q4x4ffn.gguf LM_FILE=lm-q4_0_4_4.gguf` (q6_K emb) + F16 im2col + concurrent encoders + **GGML_OPENMP=OFF** + **deferred deep stages** + **lifetime (ggml-alloc) activation buffers (early and late)** + **PIECES=2** + **zero-copy weight load** | **~3.15-3.17** | Protocol: 3.1501-3.1745 (7 runs); clip cells 17 s 2.7495, 69 s 3.0720, 138 s 3.1341; VAE 22.7 s; LM 8.9 s; RSS 2.18 GB; load 1.2 s; **40-utt gate WER 4.68%**, 40/40 transcripts byte-identical to the arena-copy build. RAM-lean alternative: p26 + the same stack = 3.2501 @ 1.90 GB, same gate |
