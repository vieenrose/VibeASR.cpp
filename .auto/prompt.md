...
| MAX-SPEED-LEAN (era v4.8, p13) | `PIECES=13 EXTRA_ENV=VAE_DEFER_LATE=1` (files as default tier; defer is REQUIRED here - without it p13 pays 2.67) | **1.45** | Protocol **1.45** - WHOLE ROW re-measured in ONE ARMED session (Exp1000), canaries 39/106/432/847 exact; 17 s 1.54, 69 s 1.61, 138 s 1.65 (the old 2.08-2.38 cells were unarmed-state), 40-utt mean 2.13 *(pre-arm, stale)*, WER 4.65 %, **RSS ~1.75 GB**. Cross-tier on real zh audio (468 CV zh-TW tokens): **b=0/c=0, bootstrap CI exactly [0,0], WER 15.38 % both** (Exp846), so the +11.8 % cost of the lean tier buys provably identical text. The boundary batch is deliberately NOT enabled here (it would cost 3 tokens in 468, p=1.0, to buy -2.6 % on a non-metric tier - Exp846's decision, revisit only if lean becomes the product default). The old seq-mode recipe (`PIECES=26 EXTRA_ENV=VAE_SEQ_ENCODERS=1`) is a LAST RESORT |
