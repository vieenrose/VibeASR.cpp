...
**Current best: 1.87 (MAX-SPEED v4.7, p1 + defer-OFF + conv-int8 + m2 tail kernel + fused gelu + fused rms_norm·gamma + in-kernel conv left pad + v4.6 final-window flush (Exp829/830) + v4.7 single-pass boundary-token prefill (Exp835)) = -84.7 %; RAM-lean
...
| MAX-SPEED-LEAN (era v4.7, p13) | `PIECES=13 EXTRA_ENV=VAE_DEFER_LATE=1` (files as default tier; defer is REQUIRED here - without it p13 pays 2.67) | **2.10** | Protocol **2.10** - WHOLE ROW re-measured in ONE session (Exp847); 17 s 2.26, 69 s 2.33, 138 s 2.40, 40-utt mean 2.13, WER 4.65 %, **RSS ~1.75 GB**. Cross-tier on real zh audio (468 CV zh-TW tokens): **b=0/c=0, bootstrap CI exactly [0,0], WER 15.38 % both** (Exp846), so the +11.8 % cost of the lean tier buys provably identical text. The boundary batch is deliberately NOT enabled here (it would cost 3 tokens in 468, p=1.0, to buy -2.6 % on a non-metric tier - Exp846's decision, revisit only if lean becomes the product default). The old seq-mode recipe (`PIECES=26 EXTRA_ENV=VAE_SEQ_ENCODERS=1`) is a LAST RESORT |
