...
**Current best: 1.19 v4.8 (MAX-SPEED, FRESH-BOOT regime, boost state = delivered 2.4 GHz 96-98 %) = -90.3 %; the SAME byte-identical binary read 1.85 on the pre-reboot long-uptime device, so ALWAYS quote the regime with a number (Exp880). RAM-lean
...
> `audit_harness.py` check 9 FAILS if a current-state prose line in this file or RESULTS.md disagrees with
> it. Read that file, not this sentence, if they differ. Added at Exp857 after this "Current best" line sat
> two eras stale (it said 2.18/v4.5 while v4.7 had shipped at 1.87) because the doc-drift refresh reached
> the measured tables but never reached the prose.
...
| **MAX-SPEED v4.8 (DEFAULT, p1, defer default OFF, flush ON, boundary batch ON, blocked-transpose cont ON)** | `VAE_FILE=vae-encoder-convint8.gguf LM_FILE=lm-q8head.gguf` (Q4_0_4x4 bulk, q6_K embeddings, Q8_0 head) + the v3.4 VAE stack (concurrent encoders + lifetime buffers + PIECES=1 + zero-copy weights + [C,T] blocks; deferred deep stages now OPT-IN - at p1 they are pure overhead, Exp708) + the v3.6-v3.8 fusions (dw-conv1d kernel, layer-scale add_scaled, gelu+bias) + **v3.9 blocked-int8 CONV weights** + **v4.2 two-column tail GEMV** (one weight pass for the ne11%4 leftovers, byte-identical, Exp765/766) + **v4.3 one-pass fused gelu+bias** (the Exp673 fusion looped the row twice; the table lookup now happens inside the bias pass - 4 tensor passes -> 2, byte-identical, Exp781) + **v4.4 fused rms_norm·gamma** (one pass instead of norm+mul, byte-identical, Exp784) + **v4.5 in-kernel causal left pad** (the per-conv `[hist|x]` pad node is deleted at cold depthwise sites, byte-identical, Exp821) + **v4.6 final-window flush** (the padded last window is gone; encode the real tail frames and feed the LM the frames that exist - a PROTOCOL change, gated not just benchmarked: 0 discordant tokens of 731, Exp829/830) + **v4.7 single-pass boundary-token prefill** (speech_start/26 frames/speech_end as ONE 28-row embedding batch instead of three decodes - a 1-row decode costs 83 ms vs 34 ms batched, Exp834/835; equivalence rests on the paired test, not on byte-identity: 38/40 transcripts identical, b=1/c=2 of 731, p=1.0) + **v4.8 blocked-transpose cont** (the [C,T]<->[T,C] stage-boundary copy was ggml's element-at-a-time fallback at 0.78 GB/s; a 16x16 blocked walk in the same branch now runs it, byte-identical, Exp863/864) | **1.19** | Protocol **1.19** (mean of 8 delivered-boost runs, sd 0.21 %, Exp942); 17 s 1.34, 69 s 1.47, 138 s 1.58 (whole ladder in ONE session, Exp939); boost-start variant 1.24/1.35/1.43/1.54 (Exp940); 40-utt mean 1.34; WER 4.55 %; RSS 2.19 GB; **state levels boost 1.192 / settled 1.333 (+11.6 %, Exp938/942)**; **40-utt gate ZERO discordant tokens of 731 vs the frozen reference (b=0/c=0, p=1.0)** (ten gates running, Exp944, 40/40 byte-identical). Protocol transcript hash `1a095c8496b4`; `FLUSH_TAIL_OFF=1` reproduces the pre-v4.6 protocol `55ac39b635cb` exactly, so every historical cell stays reachable. **The pre-reboot long-uptime device read 1.85 / 2.01 / 2.10 / 2.16 for the same binary (Exp864) - always quote the regime.** |
...
| MAX-SPEED-LEAN (era v4.8, p13) | `PIECES=13 EXTRA_ENV=VAE_DEFER_LATE=1` (files as default tier; defer is REQUIRED here - without it p13 pays 2.67) | **2.08** | Protocol **2.08** - WHOLE ROW re-measured in ONE session (Exp864); 17 s 2.24, 69 s 2.32, 138 s 2.38, 40-utt mean 2.13 *(v4.7 cell)*, WER 4.65 %, **RSS ~1.75 GB**. Cross-tier on real zh audio (468 CV zh-TW tokens): **b=0/c=0, bootstrap CI exactly [0,0], WER 15.38 % both** (Exp846), so the +11.8 % cost of the lean tier buys provably identical text. The boundary batch is deliberately NOT enabled here (it would cost 3 tokens in 468, p=1.0, to buy -2.6 % on a non-metric tier - Exp846's decision, revisit only if lean becomes the product default). The old seq-mode recipe (`PIECES=26 EXTRA_ENV=VAE_SEQ_ENCODERS=1`) is a LAST RESORT |
...
- **Secondary**: `vae_s`, `lm_s` (phase split — attack the bigger), `peak_rss_mb`
  ...
  `load_s` (model load, wall-clock only), `cpu7_deliv2400_pct` (percent of the run's
  big-core time DELIVERED at 2.4 GHz — the state variable; 96-98 = boost, 0 = settled,
  in between = a half-flipped run; the `cpu7_khz*` columns are the governor's REQUEST
  and cannot see sub-request throttling).
- **State/regime discipline** (Exp938/942/943/944): the metric has TWO clock states
  (boost 1.192 / settled 1.333 on the protocol clip, +11.6 %) and TWO device regimes
  (fresh-boot 1.19 vs pre-reboot long-uptime 1.85 for the same binary, Exp880). Quote
  the regime AND the state with any number; never compare cells from different states
  or regimes. All anchors here are idle-first, i.e. boost.
- **Record rule**: `uptime_h` in an ASI must be DERIVED from `.auto/device_state.tsv`'s
  `uptime_s` column (the single source measure.sh samples), never carried forward from
  the previous round. The Exp921-944 notes drifted to +4.5 h that way (true 19.0 h when
  they said 23.5 h) — the anchor rtfs were unaffected, only the hour labels lied.

## How to Run
`EXTRA_ENV` on measure.sh forwards env vars to the device run (e.g.
`EXTRA_ENV="VAE_SEQ_ENCODERS=1"` for the sequential-encoder fallback - 3.19 @
1.82 GB vs the default 2.79 @ 2.23 GB; the RAM-lean tier itself is `PIECES=13`
with concurrency, 2.84 @ 1.88 GB, so prefer that).
...
- Exp420 (KEEP, rotation): F16 10.50 x61, best of band. F16 nominal => Exp419
  read as jitter; formal re-check still queued. 420 runs.
- Exp421 (KEEP, watch resolved): 6.61 exceed -> immediate 6.52 re-check.
  Noise 9th time. Watch retired. 421 runs.
- Exp422 (KEEP, rotation): Q4 6.84 @31.4C, top edge. 422 runs.
- Exp423 (KEEP, rotation): ultra-lean 7.07 @31.3C, in band. 423 runs.
- Exp424 (KEEP): health 6.56 @31.3C, mid-band. Steady. 424 runs.
- Exp425 (KEEP, rotation): F16 10.49 x62, best of band. 425 runs.
- Exp426 (KEEP): health 6.55 @31.5C. Steady. 426 runs.
- Exp427 (KEEP, rotation): Q4 6.84 @31.5C, top edge. 427 runs.
- Exp428 (KEEP, rotation): ultra-lean 7.06 @31.5C, in band. 428 runs.
- Exp429 (KEEP): health 6.56 @31.4C, mid-band. Steady. 429 runs.
- Exp430 (KEEP, rotation): F16 10.53 x63 identical. 430 runs.
- Exp431 (KEEP): health 6.51 @31.5C, anchor exact. 431 runs.
- Exp432 (KEEP, rotation): Q4 6.82 @31.4C, in band (Exp427 resolved). 432 runs.
- Exp433 (KEEP, rotation): ultra-lean 7.06 @31.4C, in band. 433 runs.
- Exp434 (KEEP): health 6.57 @31.4C, mid-band. Steady. 434 runs.
- Exp435 (KEEP, rotation): F16 10.52 x64 identical. 435 runs.
- Exp436 (KEEP): health 6.52 @31.3C, anchor exact. 436 runs.
- Exp437 (KEEP, rotation): Q4 6.80 @30.8C, in band. 437 runs.
- Exp438 (KEEP, rotation): ultra-lean 7.05 @30.9C, in band. 438 runs.
...
- Exp443 (KEEP, rotation): ultra-lean 7.05 @31.2C, in band. 443 runs.
- Exp444 (KEEP): health 6.56 @31.2C, mid-band. Steady. 444 runs.
- Exp445 (KEEP, rotation): F16 10.49 x66, best of band. 445 runs.
- Exp446 (KEEP): health 6.49 @31.4C, best of band (low edge). 446 runs.
- Exp447 (KEEP, rotation): Q4 6.81 @31.4C, in band. 447 runs.
- Exp448 (KEEP, rotation): ultra-lean 7.06 @31.4C, in band. 448 runs.
- Exp449 (KEEP): health 6.57 @31.5C, mid-band. Steady. 449 runs.
- Exp450 (KEEP, rotation): F16 10.50 x67, best of band. 450 runs.
...
- Exp455 (KEEP, rotation): F16 10.50 x68, best of band, nominal despite storm.
  455 runs.
- Exp456 (KEEP): health 6.57 @33.4C, mid-band. Storm easing. 456 runs.
...
- Exp548 (KEEP, diarization smoke test): first attribution evidence. Two-speaker
  ...
  tier emits Speaker 0 + Speaker 1 while the F16 control merges them, so the
  quantization does not collapse attribution. No multi-speaker reference exists
  in the loop's assets => attribution QUALITY is unscored (validation boundary).
  Health 3.4558.
