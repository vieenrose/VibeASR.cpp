...
| **MAX-SPEED v3.4 (DEFAULT, p2)** | `VAE_FILE=vae-encoder-q4x4ffn.gguf LM_FILE=lm-q4_0_4_4.gguf` (q6_K emb) + F16 im2col + concurrent encoders + **GGML_OPENMP=OFF** + **deferred deep stages** + **lifetime (ggml-alloc) activation buffers** + **PIECES=2** + **zero-copy weight load** + **depthwise taps** + **[C,T] ConvNeXt blocks** (VAE_CT_BLOCK_OFF=1 / VAE_DW_CT_OFF=1 revert) | **~2.83** | Protocol: 2.8297; clip cells 17 s 2.4919, 69 s 2.8146, 138 s 2.8825; VAE 19.6 s; LM 8.7 s; RSS 2.18 GB; load 1.2 s; **40-utt gate WER 4.41%** (38/40 transcripts identical to the v3.3 build). RAM-lean alternative (PIECES=26 + VAE_SEQ_ENCODERS=1): see Exp587+ |
...
| MAX-SPEED-LEAN | `VAE_FILE=vae-encoder-q4x4ffn.gguf LM_FILE=lm-q4_0_4_4.gguf PIECES=26 EXTRA_ENV=VAE_SEQ_ENCODERS=1` | **~3.86-3.89** | VAE 30.0 s; **RSS 1764 MB** (lifetime allocator on the piece-wise path, Exp578; 1910 MB before); tokens 39; `VAE_LEGACY_ARENA=1` gives the bit-exact legacy variant |
