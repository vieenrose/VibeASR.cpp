...
| **MAX-SPEED v3.3 (DEFAULT, p2)** | `VAE_FILE=vae-encoder-q4x4ffn.gguf LM_FILE=lm-q4_0_4_4.gguf` (q6_K emb) + F16 im2col + concurrent encoders + **GGML_OPENMP=OFF** + **deferred deep stages** + **lifetime (ggml-alloc) activation buffers** + **PIECES=2** + **zero-copy weight load** + **depthwise conv as dense multiply-adds** (VAE_DW_CT_OFF=1 restores im2col) | **~3.08-3.09** | Protocol: 3.0829-3.0916; clip cells 17 s 2.6900, 69 s 3.0183, 138 s 3.0933; VAE 22.2-22.3 s; LM 8.7 s; RSS 2.18 GB; load 1.2 s; **40-utt gate WER 4.68%**, 40/40 transcripts byte-identical to the v3.2 gate. RAM-lean alternative (PIECES=26 + VAE_SEQ_ENCODERS=1): 3.88 @ 1.76 GB |
