...
| **MAX-SPEED v3.6 (DEFAULT, p2)** | `VAE_FILE=vae-encoder-q4x4ffn.gguf LM_FILE=lm-q8head.gguf` (Q4_0_4x4 bulk, q6_K embeddings, **Q8_0 head** - the q6_K head has no gemv kernel so each decode token pays the generic path) + the v3.4 VAE stack (F16 im2col + concurrent encoders + deferred deep stages + lifetime buffers + PIECES=2 + zero-copy weights + [C,T] blocks) | **~2.55** | Protocol: 2.5460; VAE 17.4 s; decode 3.6 s; RSS 2.23 GB; load 1.2 s; **40-utt gate WER 4.41%**, 40/40 transcripts byte-identical to the q6_K-head build (re-validated at HEAD, tag gate645; 40-utt mean 2.842). RAM-lean alternative (`PIECES=13`, encoders still concurrent): see tier row |
