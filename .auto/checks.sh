...
TEXT=$( sed -n '/--- Transcription ---/,$p' "$T" | tail -n +2 || true )   # pipefail-safe: absent line != failed run (Exp876)
...
TOP1=$( echo "$TEXT" | tr '[:upper:]' '[:lower:]' | grep -oE "[a-z0-9']+" | sort | uniq -c | sort -rn | head -n 1 | awk '{print $1}' || true )   # pipefail-safe: absent line != failed run (Exp876)
if [ "$TOP1" -gt 0 ] && [ "$(( TOP1 * 100 / NWORDS ))" -gt 40 ]; then echo "checks: word-dominance collapse ($TOP1/$NWORDS)"; exit 1; fi
NTOK=$( grep -oE 'tokens: [0-9]+' .auto/last_err.txt | head -n 1 | awk '{print $2}' || true )   # pipefail-safe: absent line != failed run (Exp876)
...
DUR=$( grep -oE 'Audio: [0-9.]+s' .auto/last_err.txt | head -n 1 | awk '{print $2}' | sed 's/s//' || true )   # pipefail-safe: absent line != failed run (Exp876)
